"""VLM backend: render each page, send to a local Qwen3-VL via OpenAI-compat API."""
...
async def _process_one(
    page: Page, *, client: VLMClient, images_dir: Path,
    asset_name: str, sem: asyncio.Semaphore,
) -> PageResult:
    try:
        async with sem:
    ...
    log.info("page %d done: %d blocks, %d assets",
        ...
        *,
        ...
        try:
            with Progress(
                ...
                for page in pages:
                    tasks.append(asyncio.create_task(_process_one(
                        page, client=client, images_dir=images_dir,
                        asset_name=asset_name, sem=sem,
                    )))
